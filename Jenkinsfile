    ...
    stages {
        ...
        stage('Test') {
            steps {
                echo 'Testing'
                script {
                    def url = 'https://test-env-jenkins-shiiki-20231119.s3.ap-northeast-1.amazonaws.com/index.html'
        ...
        stage('Release') {
            steps {
                ...
                withCredentials([[
                    ...
                    accessKeyVariable: 'AWS_ACCESS_KEY_ID',
                    secretKeyVariable: 'AWS_SECRET_ACCESS_KEY']]){
                        sh(script: 'aws s3 cp /var/lib/jenkins/workspace/JenkinsPipeline/index.html s3://prod-env-jenkins-shiiki-20231119/')
                }
            }
        }
    }
